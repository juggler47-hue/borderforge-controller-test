  ...
  async function syncGame(data){
    if(phone){
      ...
      for(const option of v?.options||[]){
        ...
        const button=document.createElement('button');button.textContent=option.kind==='capital'?`Capital: ${option.name}`:option.name;
        button.onclick=()=>{
          ...
          const confirm=document.createElement('button');confirm.textContent=`Confirm: ${option.name}${count!==undefined?' — '+count+' armies':''}`;
          const cancel=document.createElement('button');cancel.textContent='Cancel';cancel.onclick=()=>{choiceKey='';};
          gameBox.replaceChildren(confirm,cancel);
