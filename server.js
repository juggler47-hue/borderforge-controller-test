  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (!path.startsWith('/api/')) {
        const files = {'/':'index.html','/index.html':'index.html','/controller.html':'controller.html','/room.js':'room.js','/room.css':'room.css','/game-bridge.js':'game-bridge.js','/phone-map.js':'phone-map.js'};
      ...
      if (req.method === 'POST') {
        ...
        let raw='';
        for await (const chunk of req) { raw+=chunk; if (raw.length>262144) return send(res,413,{error:'Request too large.'}); }
